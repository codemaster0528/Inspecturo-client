...
const CardHighestInspecturoScoreOwner = () => {
  ...
  const getDataFromAPI = async () => {
    ...
    fetch('http://161.35.118.186/mkulima/dereva/mkaguzi', requestOptions)
      .then(response => response.json())
      .then(result => {
        result.data.sort(function (b, a) {
          return a.driverInspecturoScore - b.driverInspecturoScore
        })
  ...
  return (
    <Card sx={{ height: 500 }}>
      ...
      <CardContent sx={{ display: 'flex', alignItems: 'center', flexDirection: 'column', textAlign: 'center' }}>
        ...
        <Typography variant='h6' sx={{ mb: 1 }}>
          <p style={{ visibility: 'hidden', height: 0, marginTop: 0 }}>Johnson Warrent Buffett</p>
